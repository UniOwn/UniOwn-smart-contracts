...
contract Market is OwnableContract, ReentrancyGuardUpgradeable, IMarket {
    ...
    function mintAndCreateLendOrder(
        ...
        uint256 nftId = _mintV(uniOwnNFTAddress, oNftId, maxEndTime);
        createLendOrder(
            uniOwnNFTAddress,
            nftId,
            maxEndTime,
    ...
    function _mintV(
        ...
        address oNftAddress = IComplexUniOwnNFT(uniOwnNFTAddress)
        ...
        nftId = IComplexUniOwnNFT(uniOwnNFTAddress).mintVNft(oNftId);
    }
    ...
    function _createLendOrder(
        ...
        require(maxEndTime > block.timestamp, "invalid maxEndTime");
        require(
            minDuration <= IComplexUniOwnNFT(nftAddress).getMaxDuration(),
            "Error:minDuration > max"
        );
        require(
            IERC165(nftAddress).supportsInterface(
                type(IComplexUniOwnNFT).interfaceId
            ),
            "not UniOwnNFT"
        );
        (, , uint64 dEnd) = IComplexUniOwnNFT(nftAddress).getDurationByIndex(
            nftId,
            0
        );
        if (maxEndTime > dEnd) {
        ...
        lending.nonce = IComplexUniOwnNFT(nftAddress).getNonce(nftId);
    ...
    function fulfillOrderNow(
        ...
        (, uint64 dEnd) = IComplexUniOwnNFT(nftAddress).getDuration(durationId);
        if (endTime > dEnd) {
            endTime = dEnd;
        }
        ...
        tid = IComplexUniOwnNFT(nftAddress).mint(
            nftId,
            durationId,
            startTime,
            endTime,
        ...
        )
    {
        ...
        curRoyalty =
            (totalPrice * IComplexUniOwnNFT(nftAddress).getRoyaltyFee()) /
            E5;
    ...
    function claimRoyalty(address nftAddress, address[] calldata paymentTokens)
        ...
        nonReentrant
    {
        address payable _beneficiary = IComplexUniOwnNFT(nftAddress)
            .getBeneficiary();
